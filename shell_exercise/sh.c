...
runcmd(struct cmd *cmd)
{
    ...
    default:
        ...
        exit(-1);
    case ' ':
        // Call to execvp, which searches the environment for the binary specified by 
        // path argv[0], and executes it if found.
        if (ecmd->argv[0] == 0)
            exit(0);
    ...
    case '|':
        ...
        // designated as input, while the second is output. The left hand process will write
        // to the right hand process.
        pipe(p);
        pid = fork1();
        if (pid == 0) {
